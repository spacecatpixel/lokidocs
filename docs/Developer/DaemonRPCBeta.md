...
### COMMAND_RPC_PERFORM_BLOCKCHAIN_TEST
...
Example Response
{
  "service_node_states": [{
    ...
    "earned_downtime_blocks": "TODO(loki): Write example string",
    ...
    "version_minor": 12345,
    "version_patch": 12345,
    "votes": ["TODO(loki): Write example string"]
  }],
  "height": 234767,
  "block_hash": "bf430a3279f576ed8a814be25193e5a1ec61d3ee5729e64f47d8480ce5a2da70",
  "status": "OK",
...
### COMMAND_RPC_GET_SERVICE_NODE_STATUS
 ...
 * `service_node_state - cryptonote::COMMAND_RPC_GET_SERVICE_NODES::response_t::entry`: Service node registration information
 * `height - uint64`: Current block's height.
 * `block_hash - string`: Current block's hash.
 * `status - string`: Generic RPC error code. "OK" is the success value.
 * `as_json - string`: If `include_json` is set in the request, this contains the json representation of the `entry` data structure
...
{
  "service_node_states": [{
    "requested_fields": "TODO(loki): Write example string",
    "service_node_pubkey": "TODO(loki): Write example string",
    "registration_height": 234767,
    "registration_hf_version": 12345,
    "requested_unlock_height": 123,
    "last_reward_block_height": 234767,
 ...
 * `fields - requested_fields_t`
   * `explicitly_set - bool = false`: internal use only: incicates whether one of the other parameters has been explicitly set
...
### COMMAND_RPC_STORAGE_SERVER_PING
 ...
 * `version_major - int`: Storage Server Major version
 * `version_minor - int`: Storage Server Minor version
 * `version_patch - int`: Storage Server Patch version
...
Constants: 
 * `NUM_BLOCKS_TO_SCAN_BY_DEFAULT - constexpr static uint32_t = 720`
 * `HEIGHT_SENTINEL_VALUE - constexpr static uint64_t = (UINT64_MAX - 1)`
...
### COMMAND_RPC_REPORT_PEER_SS_STATUS
...
-d @- << EOF
{
  ...
  "params": {
    ...
    "pubkey": "TODO(loki): Write example string",
    "passed": true
  }
}
EOF
...
### COMMAND_RPC_LNS_NAMES_TO_OWNERS
...
Constants: 
 * `MAX_REQUEST_ENTRIES - static size_t const = 256`
 * `MAX_TYPE_REQUEST_ENTRIES - static size_t const = 8`
...
### COMMAND_RPC_LNS_OWNERS_TO_NAMES
...
-d @- << EOF
{
  ...
  "params": {
    "entries": ["059f5a1ac2d04d0c09daa21b08699e8e2e0fd8d6fbe119207e5f241043cf77c30d"]
  }
}
EOF
 ...
 * `entries - response_entry[]`
   * `request_index - uint64`: The index in request's `entries` array that was resolved via Loki Name Service.
