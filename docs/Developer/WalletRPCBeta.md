 ...
 * `priority - uint32`: Set a priority for the transaction. Accepted values are: or 0-5 for: default, unimportant, normal, elevated, priority, blink.
 * `blink - bool`: Set to true to request blink. In the next release this will be deprecated and setting priority to 5 for blink will be the only step required.
 ...
 * `priority - uint32`: Set a priority for the transaction. Accepted values are: or 0-5 for: default, unimportant, normal, elevated, priority, blink.
 * `blink - bool`: Set to true to request blink. In the next release this will be deprecated and setting priority to 5 for blink will be the only step required.
 * `ring_size - uint64`: (Deprecated) Sets ringsize to n (mixin + 1). Loki ring_size is statically set to 10.
